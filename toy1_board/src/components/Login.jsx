...
import { FormWrapper, Label, Input, SubmitButton, ErrorMessage} from './styles/FormStyles';
...
const Login = ()=>{
    ...
    const handleLoginSubmit = async (e) => {
        ...
        if(
            (loginId === storeUserData.username || loginId === storeUserData.email) &&
    ...
    return(
        <FormWrapper>
            ...
            <form onSubmit={handleLoginSubmit} method="">
                ...
                <Input 
                    className="login_id" type="text"  id="login_id" name="id"
                    value={loginId} placeholder="아이디 또는 이메일을 입력하세요"
                    onChange={(e) => setLoginId(e.target.value)}></Input>
                <Label htmlFor="login_pw">비밀번호</Label>
                <Input
                    className="login_pw" type="password"  id="login_pw" name="password"
                    value={password} placeholder="비밀번호를 입력하세요"
                    onChange={(e)=> {setPassword(e.target.value)}}></Input>
                <SubmitButton type="submit" onClick={handleLoginSubmit} disabled={isLoggingIn}>
